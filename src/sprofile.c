...
// TODO: need timer implementation for Windows
#else
#include <signal.h>
...
DLLEXPORT int jl_sprofile_start_timer(void)
{
...
DLLEXPORT void jl_sprofile_stop_timer(void)
{
    running = 0;
}
#else
...
// Utility functions
//
DLLEXPORT int jl_sprofile_init(size_t maxsize, u_int64_t delay_nsec)
{
    bt_size_max = maxsize;
    nsecprof = delay_nsec;
...
DLLEXPORT u_int8_t* jl_sprofile_get_data(void)
{
    return (u_int8_t*) bt_data_prof;
}

DLLEXPORT size_t jl_sprofile_len_data(void)
{
    return bt_size_cur;
}

DLLEXPORT size_t jl_sprofile_maxlen_data(void)
{
    return bt_size_max;
}

DLLEXPORT void jl_sprofile_clear_data(void)
{
    bt_size_cur = 0;
}
